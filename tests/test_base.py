...
def test_config_defaults(base_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    for apin in arduino.io_map[ConfigPinTypes.ANALOG_INPUTS]:
        ...
        assert apin.pinConnectedState == PinConfigElement.PIN_CONNECTED_STATE.value[1]
        assert apin.pinDisconnectedState == PinConfigElement.PIN_DISCONNECTED_STATE.value[1]

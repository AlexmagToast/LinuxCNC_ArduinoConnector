...
class MsgPacketizer:
    ...
    def feed(self, buffer: bytearray):
        ...
        while self._recv_bytes.find(b"\x00") != -1:
            ...
            #logging.debug(f"cobs encoded1: {chunk}, rest: {self._recv_bytes}")
            #if chunk.find(b'\r\n') != -1:
            #    [chunk2, chunk] = chunk.split(b'\r\n', maxsplit=1)
            #    print(chunk2)
            #    pass
            #logging.debug(f"cobs encoded2: {chunk}, rest: {self._recv_bytes}")
